...
def main():
    ...
    if choice == 'Home':
        ...
        k_nbrs = NearestNeighbors(
            algorithm='brute', n_jobs=-1, n_neighbors=10, metric='cosine')
        k_nbrs.fit(data)

        if status:
            if st.button('Recommend'):
                r = pd.DataFrame()
                r = get_recomd_startups(main_data, get_idx(
                    data, k_nbrs, status, vote, n_startups))
                st.write(r)
        ...
        st.write(
            'Application is developed by Bebeto Nyamwamu to recommend startups to investors')
        st.write(
            " For the report and more [read](https://drive.google.com/file/d/1OtYSzBA_Sjrw4H_EGeDsRD9yO7aw3IpP/view?usp=sharing)")
        st.warning(
            'Warning! `Nuff said: To talk to your Data Scientist/Machine Learning Engineer at :email: [email](mailto:nberbetto@gmail.com) or view his works/profile on [Github](https://github.com/realonbebeto) or [Portfolio](http://realonbebeto.github.io/)')
...
def take_inputs():
    status = st.multiselect(
        'Status', ['acquired', 'closed', 'ipo', 'operating'], default=['acquired'])
...
def get_idx(data, model, status: list, vote: tuple, n_startups: int):
    ...
    if len(status) == 1:
        ...
        startup_list = data.loc[(data['status'] == status_dict[status[0]]) | (
            data['status'] == status_dict[status[1]])]
    else:
        startup_list = data.loc[(data['status'] == status_dict[status[0]]) | (
            data['status'] == status_dict[status[1]]) | (data['status'] == status_dict[status[2]])]

    rt = list(startup_list.loc[(startup_list['avg_vote'] >= vote[0]) & (
        startup_list['avg_vote'] <= vote[1])].index)
    if rt:
        if (n_startups > 0):
            picked = random.choice(rt)
            recom_idx = model.kneighbors(X=data.iloc[picked].values.reshape(
                1, -1), n_neighbors=n_startups, return_distance=False)
            return recom_idx
        elif (n_startups < 0):
            st.warning(
                'Invalid: Negative Number! Change the number of startups to recommend')
...
def get_recomd_startups(data, ridx: np.ndarray):
    ...
    if ridx.any():
        if len(ridx) > 0:
            result_recom = data.loc[ridx[0].tolist(), [
                'normalized_name', 'category_code', 'country_code', 'relationships', 'avg_vote']].copy()
            result_recom.columns = ['Name', 'Category',
                                    'Country', 'Employees', 'Chance']
            result_recom.fillna('NA', inplace=True, axis=1)
            return result_recom

    elif ridx == None:
        st.warning(
            'Try changing the status, chance of success and number of startups to recommend')
